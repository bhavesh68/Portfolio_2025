...
// import { assets, miniWorkData} from '@/assets/assets'
...
//         {miniWorkData.map((project, index) => (
//           <motion.div
        ...
        {miniWorkData.map((project, index) => (
          project.link ? (
            ...
            <motion.div
              ...
              whileHover={{ scale: 1.05 }}
              transition={{ duration: 0.3 }}
              className='aspect-square bg-no-repeat bg-cover bg-center rounded-lg relative cursor-not-allowed group mini-card opacity-50'
              style={{ backgroundImage: `url(${project.bgImage})` }}>
              <div className='bg-white w-10/12 rounded-md absolute bottom-5 left-1/2 -translate-x-1/2 py-3 px-5 flex items-center justify-between duration-500 group-hover:bottom-7'>
                <div>
                  <h2 className='font-semibold'>{project.title}</h2>
                ...
                <div className='border rounded-full border-black w-9 aspect-square flex items-center justify-center shadow-[2px_2px_0_#000] transition opacity-50'>
                  <Image src={assets.send_icon} alt='send icon' className='w-5'/>
                </div>
              </div>
            </motion.div>
...
//below code is from github ogn repo
